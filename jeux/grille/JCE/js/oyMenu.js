...
oyCrosswordMenu.prototype.installWelcomeMenu = function(){
	...
	this.addNoneWordAction( 
		target, 
		"Bienvenue <a class='oysTextLink' href='' id='oygWelcomeLink'>" + "" + "</a>! "
	);		
	...
	this.addAction( 
		target, "Commencer", "Démarrage...", "strt",
		function(){				 
			oThis.puzz.bind();	
			...
			oThis.footer.stateOk("Bonne grille !");
		}
	);	

	this.footer.stateOk("Prêt !");
...
oyCrosswordMenu.prototype.installContextMenu = function(){
	...
	if (!this.canReveal){
		...
		if (hidx != -1){
			var caption = "Montrer <b>" + (hidx + 1) + "Horiz.</b>"
		...
		if (vidx != -1){
			var caption = "Montrer <b>" + (vidx + 1) + "Vert.</b>";
	...
	if (!this.canCheck){
		...
		var caption = "Vérifier <b>" + (hidx + 1) + "Horiz.</b>";
		if (hidx != -1){
			if (!this.hlist.clues[hidx].completed()){
		...
		var caption = "Vérifier <b>" + (vidx + 1) + "Vert.</b>";
		if (vidx != -1){
			if (!this.vlist.clues[vidx].completed()){	 
				this.addCheckWordAction(
					this.vlist.clues[vidx], target, caption
		...
		var oThis = this;
		this.addAction(target, "<b>Tout</b> vérifier", "Vérification en cours...", "chkll",
			function(){				
				oThis.checkAll(); 
				oThis.invalidateMenu();		
				return false; 
...
oyCrosswordMenu.prototype.installDoneMenu = function(){	
	...
	this.addNoneWordAction(target, "Grille complète !");	 
	this.addNewLine(target);	     
	
	var msg = "Vous avez obtenu <b>" + this.score + "</b> points";
	if (this.rank != -1){
		msg += " (rang <b>" +  this.rank + "</b>)";
	}  
	msg += ".";
	...
	this.footer.stateOk("Fin de la grille !");
...
oyCrosswordMenu.prototype.addSubmitLeaveMenuItems = function(target){
	...
	var oThis = this;
	this.addAction(target, "Quitter", "Fermeture...", "lv",
		function(){			
			oThis.leaveGameEarly(oThis.puzz.leaveGameURL);
			oThis.footer.stateOk("Fini");
			return false; 
		} 
	);
}
...
oyCrosswordMenu.prototype.leaveGameEarly = function(url){
	this.footer.stateBusy("Fermeture...");

	var canLeave = true;
	if (this.puzz.started && !this.over){
		canLeave = confirm("Grille incomplète. Quitter tout de même ?");
	}	  
	if (canLeave){ 
		window.location = url;
	}
	
	this.footer.stateOk("Fini");
}
...
oyCrosswordMenu.prototype.addCheckWordAction = function(clue, target, caption){
	var oThis = this;
	this.addAction(target, caption, "Vérification...", "chk",
		function(){				
...
oyCrosswordMenu.prototype.addRevealWordAction = function(clue, target, caption){
	var oThis = this;
	this.addAction(target, caption, "Solution...", "rvl",
		function(){				
...
oyCrosswordMenu.prototype.revealWord = function(clue){
	...
	this.footer.stateOk("Montré : [" + status.buf + "]!");
}  
...
oyCrosswordMenu.prototype.checkAll = function(){
	...
	if  (checked == 0){
		this.footer.stateError("Mots incomplets !");
	} else {
		this.footer.stateOk("Vérifié : " + checked + ", " + correct + " est juste !"); 
	}
}  
...
oyCrosswordMenu.prototype.checkWord = function(clue){
	var status = this.checkWordStatus(clue);	  
	if (!status.isComplete){
		this.footer.stateError("Le mot [" + status.buf + "] est incomplet !");
	} else { 
		this.checks++; 
		this.deducts += this.getDeductionForCheck(clue);			
		if (status.wrong != 0){		  
			this.footer.stateError("[" + status.buf + "] ne convient pas !");
		} else { 
			this.matches++; 
			...
			this.footer.stateOk("[" + status.buf + "] est juste !");
		}
	}
}
