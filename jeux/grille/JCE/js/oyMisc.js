...
oyCrosswordFooter.prototype.clockUpdate = function(){	
	...
	if (this.puzz.started){
		...
		document.getElementById("oygFooterClock").innerHTML = "&nbsp;Durée: <b>" + pad(min) + "</b>:<b>" + pad(sec) +"</b>";
	}
} 
...
oyCrosswordFooter.prototype.update = function(){	
	...
	if (!this.puzz.started){
		buf += "La résolution n'a pas encore commencé !";
	} else {
		buf += "Score: <b>" + this.puzz.menu.score + "</b> points"; 		
		if(this.puzz.menu.rank != -1){
			buf += " (rang <b>" + this.puzz.menu.rank + "</b>)"; 		
		}
	}
...
/* Fonction adaptée et actualisée par Patrick Cardona */
function oyBrowserDetection(){
	...
	this.isSafari=false; // pour safari
	this.isChrome=false; // pour Chrome
	this.isGood=false;
	this.sf=false;
	...
	if (
		...
		(this.isSafari && this.version >=5) ||
		(this.isChrome && this.version >=17)
	){	
		this.isGood = true;	
	} 
}
...
function oygBind(puzz){
	...
	if (!isSupported){  
		var msg =  
			"Votre navigateur ne permettra peut-être pas de charger cette grille.\n" + 
			"Testé avec des versions de Microsoft IE 6.0+, Firefox 1.0+,\n" +
			" Netscape6.0+, Safari 5+, Chrome 17+.\n\n" +
	...
	if (!isSupported){ 
		oygError = "Ce navigateur ne semble pas supporté.";
	} else {
		var div = document.getElementById("oygContext");
		if (div == null){
			oygError = "Erreur de modèle.";
		} else {			
			var frame = document.getElementById("oygPuzzle");
			if (frame == null){
				oygError = "Echec dans le chargement de la grille.";
			} else {
				puzz.init();
				puzz.render();
